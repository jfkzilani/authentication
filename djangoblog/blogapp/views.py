...
def getRegister(request):
    ...
    if form.is_valid():
        ...
        mail_subject = "Confirmation for registration"
        ...
        to_email = form.cleaned_data.get('email') #name of email field in form
        to_list = [to_email]
        from_email = settings.EMAIL_HOST_USER
        send_mail(mail_subject, message, from_email, to_list, fail_silently=True)
        return HttpResponse("<h2>Thanks for your registration. A confirmation link was sent to your mail</h2>")
...
def activate(request, uid, token):
    ...
    if user is not None and activation_token.check_token(user, token):
        user.is_active=True
        user.save()
        return HttpResponse("<h1>Account is activated. Please login <a href='/login'>Login</a></h1>")
    else:
